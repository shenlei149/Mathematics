...
所以$w(h_i)<w(t_i)$。  
令$T_{i-1}$是第$i-1$步后得到的森林，$H_i$是边$h_1,h_2,\cdots,h_i$组成的森林，根据**Lemma 10.11**一定存在一条边$h_j,j\leq i$可以添加到$T_{i-1}$中而不形成环，又有$w(h_j)\leq w(h_i)<w(t_i)$，所以贪婪算法的第$i$步不会选择$t_i$。这和之前的假设矛盾。所以不存在这样的$H$，那么贪婪算法找到的$T$就是权重最小生成树。
