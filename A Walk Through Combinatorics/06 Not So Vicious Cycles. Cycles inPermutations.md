...
(24) 令$SQ(n)$至少有一个平方根，证明$|SQ(2n)|\cdot(2n+1)=|SQ(2n+1)|$。注意，这个公式的含义是$p(2n)=p(2n+1)$，其中$p(m)$是$m$-排列有平方根的概率。  
**Solution** 构造从$SQ(2n)\times [2n+1]$到$SQ(2n+1)$的双射$\kappa$。这个和Lemma 6.27.的$\Psi$类似，扩展奇数循环的排列。  
令$(\pi,k)∈SQ(2n)\times[2n+1]$。将$\pi$分成奇数循环和偶数循环。如果间隔位置$k$在奇数循环中或者是最后一个位置，应用$\Psi$在这些奇数循环中，连通其他偶数循环，得到$\kappa(k)$。$2n+1$还是在某个奇数循环中。  
